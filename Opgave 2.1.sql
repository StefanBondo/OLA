...
CREATE TABLE seller (
    seller_cvr     VARCHAR(8)  PRIMARY KEY,     -- bruges som nøgle for sælger
...
CREATE TABLE car (
    carid      BIGINT       PRIMARY KEY,        -- samme navn som i dit billede
...
CREATE TABLE car_observation (
    ...
    `Sys.time..` DATETIME(6) NOT NULL,  -- bemærk backticks pga. punktummer
    price       VARCHAR(22) NOT NULL,
    details     VARCHAR(47),
    properties  VARCHAR(255),   -- sørg for at den er mindst lige så stor som i jeres rå tabel
    description TEXT,
